...
protected:                                                                  // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        size_t min;
        auto min_floor = (size_t)floorl(static_cast<long double>(tableSize) / maxLoadFactor + 1);
        if (min_floor >= bucketSize) min = min_floor;
        else min = bucketSize;
        if (min > HashPrime::g_a_sizes[HashPrime::num_distinct_sizes_64_bit - 1]) throw std::range_error("range_error");
        else return *lower_bound(HashPrime::g_a_sizes, HashPrime::g_a_sizes + HashPrime::num_distinct_sizes_64_bit - 1, min);
    }
...
public:
    ...
    HashTable(const HashTable &that) {
        ...
        for (int i = 0; i < buckets.size(); i++) this->buckets[i] = that.buckets[i];
    ...
    HashTable &operator=(const HashTable &that) {
            this->tableSize = that.tableSize;
            this->firstBucketIt = that.firstBucketIt - that.buckets.begin() + this->buckets.begin();
            this->buckets.clear();
            this->buckets.resize(that.buckets.size());
            for (int i = 0; i < buckets.size(); i++) this->buckets[i] = that.buckets[i];
            this->keyEqual = that.keyEqual;
    ...
    Iterator find(const Key &key) {
        size_t pos = hashKey(key, bucketSize());
        auto before = buckets[pos].before_begin();
        auto point = buckets[pos].begin();
        while (point != buckets[pos].end()) {
            if (keyEqual(point->first, key)) {
                Iterator it(this, buckets.begin() + pos, before);
                it.endFlag = false;
                return it;
            }
            point++;
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value) {
        ...
        if (!it.endFlag) {
            ...
            it.bucketIt->insert_after(it.listItBefore, node);
            this->tableSize++;
            if (it.bucketIt < this->firstBucketIt) this->firstBucketIt = it.bucketIt;
            if (this->loadFactor() > this->getMaxLoadFactor()) rehash(this->buckets.size() + 1);
            return true;
        }
        // TODO: implement this function
    }
    ...
    bool insert(const Key &key, const Value &value) {
        auto it = find(key);
        return this->insert(it, key, value);
        // TODO: implement this function
    }
    ...
    void rehash(size_t bucketSize) {
        ...
        HashTableData new_data(this->buckets);
        this->buckets.clear();
